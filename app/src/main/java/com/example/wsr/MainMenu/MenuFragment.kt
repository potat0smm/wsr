...
class MenuFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        FL = binding.FM
        thirdMenuAdapter = MenuAdapterThird(emptyList(), addBtn, FL, sharedViewModel)
        recyclerView.adapter = thirdMenuAdapter

        recyclerView = binding.secondRecyclerView
        recyclerView.layoutManager = LinearLayoutManager(requireContext(),RecyclerView.HORIZONTAL,false)
        secondMenuAdapter = MenuAdapterSecond(sharedViewModel)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            if (response.isSuccessful && !catalog.isNullOrEmpty()) {
                ...
                sharedViewModel.updateSelectedItem(catalog[0]) // Обновляем данные в SharedViewModel с передачей первого элемента из списка
            } else {
                // Обработка ошибки
            }
        }

        // Создайте экземпляр класса MenuAdapter
        ...
        lifecycleScope.launch {
            ...
            if (responseFirst.isSuccessful) {
                ...
                // Присвойте данные и вызовите notifyDataSetChanged() на menuAdapter
